...
# Plan 01 — Collect final evidence inventory and reproducibility artifacts

<objective>
Collect final evidence inventory and reproducibility artifacts
</objective>
...
<task id="P6-01-T1" type="execute">
  ...
  <action>
    Create or update the minimal files needed for this plan objective: `Collect final evidence inventory and reproducibility artifacts`. Keep names explicit and avoid hidden behavior. Preserve outputs from earlier phases.
  </action>
  <acceptance_criteria>
    - `test -d .planning/phases/07-final-evidence-and-ieee-report` exits 0.
    - At least one project file or documentation artifact relevant to `01-evidence-freeze` exists after execution.
  </acceptance_criteria>
</task>
...
<task id="P6-01-T2" type="execute">
  ...
  <action>
    Implement the smallest working slice for `Collect final evidence inventory and reproducibility artifacts`. If the slice depends on external credentials, network access, dataset availability, gated model access, or prior phase runtime artifacts, document the exact blocker and fallback in the corresponding docs/artifacts file instead of guessing.
  </action>
  <acceptance_criteria>

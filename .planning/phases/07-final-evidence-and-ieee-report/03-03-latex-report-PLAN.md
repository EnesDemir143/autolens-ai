...
plan_id: 07-03
objective: Generate IEEE LaTeX report with the local report creator skill
wave: 2
...
# Plan 03 — Generate IEEE LaTeX report with the local report creator skill

<objective>
Generate IEEE LaTeX report with the local report creator skill
</objective>
...
<task id="P6-03-T1" type="execute">
  ...
  <action>
    Create or update the minimal files needed for this plan objective: `Generate IEEE LaTeX report with the local report creator skill`. Keep names explicit and avoid hidden behavior. Preserve outputs from earlier phases.
  </action>
  <acceptance_criteria>
    - `test -d .planning/phases/07-final-evidence-and-ieee-report` exits 0.
    - At least one project file or documentation artifact relevant to `03-latex-report` exists after execution.
  </acceptance_criteria>
</task>
...
<task id="P6-03-T2" type="execute">
  ...
  <action>
    Implement the smallest working slice for `Generate IEEE LaTeX report with the local report creator skill`. If the slice depends on external credentials, network access, dataset availability, gated model access, or prior phase runtime artifacts, document the exact blocker and fallback in the corresponding docs/artifacts file instead of guessing.
  </action>
  <acceptance_criteria>

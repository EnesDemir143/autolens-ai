...
wave: 1
depends_on: []
requirements_addressed: ['DATA-01', 'DATA-02', 'DATA-04']
files_modified: ['src/autolens_ai/data/', 'configs/data/', 'artifacts/dataset/', 'docs/dataset_manifest.md']
autonomous: true
---

# Plan 01 — Catalog candidate Kaggle/HF sources and legal/source metadata

<objective>
Catalog candidate Kaggle/HF sources and legal/source metadata
</objective>

<must_haves>
- Covers requirement IDs: DATA-01, DATA-02, DATA-04.
...
<task id="P2-01-T1" type="execute">
  ...
  <action>
    Create or update the minimal files needed for this plan objective: `Catalog candidate Kaggle/HF sources and legal/source metadata`. Keep names explicit and avoid hidden behavior. Preserve outputs from earlier phases.
  </action>
  <acceptance_criteria>
    - `test -d .planning/phases/02-dataset-research-and-curation` exits 0.
    - At least one project file or documentation artifact relevant to `01-source-catalog` exists after execution.
  </acceptance_criteria>
</task>

<task id="P2-01-T2" type="execute">
  ...
  <action>
    Implement the smallest working slice for `Catalog candidate Kaggle/HF sources and legal/source metadata`. If the slice depends on external credentials, network access, dataset availability, gated model access, or prior phase runtime artifacts, document the exact blocker and fallback in the corresponding docs/artifacts file instead of guessing.
  </action>
  <acceptance_criteria>
    - Output references all requirement IDs: DATA-01, DATA-02, DATA-04.

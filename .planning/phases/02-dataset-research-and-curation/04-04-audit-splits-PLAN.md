...
# Plan 04 — Generate manifest, balance audit, dedup report, and split files

<objective>
Generate manifest, balance audit, dedup report, and split files
</objective>
...
<task id="P2-04-T1" type="execute">
  ...
  <action>
    Create or update the minimal files needed for this plan objective: `Generate manifest, balance audit, dedup report, and split files`. Keep names explicit and avoid hidden behavior. Preserve outputs from earlier phases.
  </action>
  <acceptance_criteria>
...
<task id="P2-04-T2" type="execute">
  ...
  <action>
    Implement the smallest working slice for `Generate manifest, balance audit, dedup report, and split files`. If the slice depends on external credentials, network access, dataset availability, gated model access, or prior phase runtime artifacts, document the exact blocker and fallback in the corresponding docs/artifacts file instead of guessing.
  </action>
  <acceptance_criteria>
